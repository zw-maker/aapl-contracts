...
contract AaplOracle is Ownable, ExampleOracleSimple, IOracle {

    uint256 constant SCALE = 10 ** 18;
    address ourCoin;
    address constant uniFactory = 0x5C69bEe701ef814a2B6a3EDD4B1652CB9cc5aA6f;

    constructor(address ourCoin_, address susd_) public ExampleOracleSimple(uniFactory, ourCoin_, susd_) {
    ...
    function getData() override external returns (uint256, bool) {
        require(msg.sender == UFragmentsI(ourCoin).monetaryPolicy());
        update();
        uint256 price = consult(ourCoin, SCALE); // will return 1 AAPL in sUSD
